...
describe('openDb', () => {
  ...
  // The local eval store (#1094 eval_run/eval_result tables) was retired in E8
  // (#1151) — eleatic's eval.sqlite is now the SOLE eval store, so this review
  // store no longer carries those tables. The "four contract tables" test above
  // already excludes them, so it is the standing guard that they stay out.
