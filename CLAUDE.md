...
2. Open a PR (`gh pr create`). **The PR description MUST follow `.github/PULL_REQUEST_TEMPLATE.md`** — all five sections (Summary, Plan/Task reference, Test plan, Review focus, Diagram) are mandatory. GitHub applies the template to UI-created PRs automatically; when using `gh pr create --body`, paste the template body verbatim and fill every section. Skipping sections wastes the bot's review cycles because the diagram-first section is load-bearing for fast architectural review.
...
**Mergify config compatibility:** `.mergify.yml` uses `queue_rules[].checks_timeout` and `merge_queue.max_parallel_checks: 1` with `batch_size: 1` so it works with branch protection's `strict: true` (require branches up-to-date). If branch protection's strictness is ever flipped off, those settings can be relaxed.
