...
**Knip false-positive workflow.** When a knip finding turns out to be a false positive (file is referenced by Terraform `file(...)`, by an npm-script bin invocation, by a tsc test config, or by any other static-analysis-invisible mechanism), DO NOT delete the file. Add an ignore rule to `knip.ts` with a dated comment matching the existing convention: (a) what it silences, (b) what class of finding it risks missing, (c) how a future re-audit verifies the rule is still warranted. Common false-positive classes: Terraform-loaded files, type-only test files compiled via `tsc -p`, npm-script bin invocations not in `package.json` deps, dynamic require/import. **Important Mergify gotcha:** branch protection has `knip (informational)` listed as a required check even though the workflow runs `continue-on-error: true`. A knip failure therefore blocks Mergify queue progression on every PR. Fix the finding (add ignore rule or address real dead code) on the same branch before posting `@Mergifyio queue`, or merge a separate `chore: knip` PR first to unblock downstream PRs.
