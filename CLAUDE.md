...
| **Transient** | Popovers (flip/shift/clamp), detail card (insets top-right region; includes the **Sightings Log** — a per-sighting section of the detail card, #1299, spec §4.5.1, NOT a new surface), filters panel (anchored under trigger) | per-element caps |
...
We track drift between artifacts and reality with the `drift:*` label taxonomy below. The mechanism is intentionally lightweight: deterministic checks (syncpack, terraform-plan-drift-check) catch structural drift; the PR-review bot (`julianken-bot`) catches narrative drift on PRs touching drift-prone surfaces; a nightly workflow (planned, see #307) catches time-emergent drift.
...
**Kill-threshold metric**: evaluate at the 60-day mark after first ship of any drift mechanism (R13 PR-bot rubric or nightly workflow). Compute `closed-as-fixed / closed-total` over the calendar month. If it drops below 40% — i.e., 6 of 10 drift issues close as FP / won't-fix / stale-signal — alert fatigue is realized. Response: comment out the nightly's `on:` block, downgrade LLM-driven rules to silent-log mode, keep deterministic checks (syncpack, terraform-plan-drift-check) intact, and file a retrospective at `docs/analyses/<date>-drift-system-retrospective.md`. The audit at `docs/analyses/2026-04-27-codebase-drift-audit/report.md` records the system's current falsifiability claim.
...
**Orphan-className check** (`scripts/ci/check-orphan-classnames.sh`, CI: `.github/workflows/orphan-classname-check.yml`): catches JSX `className` references with no matching CSS selector rule. Ships in soft-launch mode (`continue-on-error: true`) for 7 days from 2026-05-11; flip to `false` after a clean-run review and add `"check-success = orphan-classname-check"` to `.mergify.yml` queue_conditions (branch-protection admin op required in parallel). Check name in GitHub UI: `orphan-classname-check`.

False-positive workflow for orphan-className check: if a className is genuinely dynamic (template literal suffix driven by a non-union runtime value), add it to `.github/orphan-classname-allowlist.yml` with (a) the component file and line, (b) why static extraction is not feasible, (c) the date added. If ternary or union-expansion patterns produce false positives, fix the script's extraction logic instead of allow-listing. Kill-threshold: same 60-day / 40% metric as the other drift mechanisms (see **Kill-threshold metric** above). If the allow-list grows past 10 entries without matching CSS rules, extend the script's static-extraction coverage.
