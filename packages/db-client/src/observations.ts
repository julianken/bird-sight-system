...
/**
 * Hard cap on the per-observation feed response (issue #647).
 *
 * Rationale (hardcoded, not env-var):
 *   - KISS: zero config surface, zero per-env divergence.
 *   - The frontend's react-window virtual list renders ~16 rows at a time
 *     regardless of payload size, so 500 is effectively unlimited for the
 *     interactive view (~30 scroll-pages deep).
 *   - Future tuning is a separate concern. The cap-hit structured log line
 *     in services/read-api/src/app.ts is the escape hatch: when the
 *     question "should we raise the cap?" becomes pressing, it converts
 *     into a Cloud Logging metric-extractor config, not a code change.
 *   - An env var would risk runtime-divergence across environments
 *     (staging vs prod cap drifting silently) — the structured log keeps
 *     observability without that hazard.
 */
export const OBSERVATIONS_FEED_LIMIT = 500;

export interface ObservationsFeedResult {
  rows: Observation[];
  totalCount: number;
  truncated: boolean;
}
...
/**
 * Capped per-observation feed for the read-api (issue #647).
 *
 * Returns at most OBSERVATIONS_FEED_LIMIT rows, plus the total matching
 * row count via `COUNT(*) OVER ()` (window-aggregated in the same query
 * so we don't issue a second round-trip). When totalCount > limit, the
 * `truncated` flag tells the API layer to surface a banner to the user.
 *
 * Decision (per #647 spec, locked a priori): include the count via window
 * function. At AZ scale this is single-digit ms; the displayed total is
 * materially more useful than "showing the most recent 500" (gives the
 * user calibration for how aggressively to filter). Re-evaluate post-flip
 * if p95 latency moves measurably.
 *
 * The aggregated-mode (zoom < 6) path is unaffected — that branch calls
 * getObservationsAggregated, which bucket-aggregates and never returns
 * raw rows.
 */
export async function getObservationsFeed(
  pool: Pool,
  f: ObservationFilters,
): Promise<ObservationsFeedResult> {
  const conditions: string[] = [];
  const params: unknown[] = [];
  ...
  // COUNT(*) OVER () emits the unfiltered-by-LIMIT total on every row.
  // We read it from row[0] (window aggregate is invariant per row).
  const limitIdx = params.length + 1;
  params.push(OBSERVATIONS_FEED_LIMIT);
  ...
  // total_count comes back as a string (pg returns bigint as string).
  // When zero rows match, the empty result set has no row to read from —
  // totalCount is 0.
  const totalCount = rows.length === 0 ? 0 : Number(rows[0]!.total_count);

  const mapped: Observation[] = rows.map(r => ({
    subId: r.sub_id,
    speciesCode: r.species_code,
    comName: r.com_name ?? r.species_code,
    lat: r.lat,
  ...
  return {
    rows: mapped,
    totalCount,
    truncated: totalCount > OBSERVATIONS_FEED_LIMIT,
  };
}
