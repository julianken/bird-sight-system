...
// ── #647 — feed-cap banner ─────────────────────────────────────────────────
describe('FeedSurface — truncation banner (#647)', () => {
  const oneObs = [obs({ subId: 'S1', speciesCode: 'vermfly', comName: 'Vermilion Flycatcher' })];

  it('renders the cap-hit banner with formatted total when truncated=true', () => {
    render(
      <FeedSurface
        loading={false}
        observations={oneObs}
        now={NOW}
        filters={{ notable: false, since: '14d', speciesCode: null, familyCode: null }}
        onSelectSpecies={() => {}}
        truncated={true}
        totalCount={12345}
      />
    );
    const banner = screen.getByTestId('feed-truncated-banner');
    expect(banner).toBeInTheDocument();
    // Locale-formatted total (12,345) per the bot's worked example.
    expect(banner.textContent).toBe(
      'Showing 500 of 12,345 observations — narrow your filters or zoom in to see more'
    );
    // a11y: the banner announces politely on filter changes.
  ...
  it('does NOT render the banner when truncated=false', () => {
    ...
    expect(screen.queryByTestId('feed-truncated-banner')).toBeNull();
  });

  it('does NOT render the banner when truncated/totalCount props are omitted (backward-compat)', () => {
    render(
      <FeedSurface
        loading={false}
        observations={oneObs}
