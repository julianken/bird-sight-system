#!/usr/bin/env node
/**
 * scripts/curation/curate-phylopic.mjs — Issue #245 (epic #251), retry hardening #267.
 *
 ...
 *   - `scripts/curation/phylopic-picks.json` — full candidate list per family + the
 ...
 *
 * Usage:
 *   node scripts/curation/curate-phylopic.mjs                       # 25-family seed; UPDATE-mode (migration 17000)
 *   node scripts/curation/curate-phylopic.mjs --refresh             # bypass cache, re-fetch all
 *   node scripts/curation/curate-phylopic.mjs --backfill            # 38-family backfill; INSERT-mode (migration 34000)
 *   node scripts/curation/curate-phylopic.mjs --recurate-nulls      # 14 NULL-svg families; UPDATE-mode (migration 35000, issue #498)
 *   node scripts/curation/curate-phylopic.mjs --rescue-via-species  # 14 NULL-svg families, species/genus cascade; UPDATE-mode (migration 36000, issue #500)
 *   node scripts/curation/curate-phylopic.mjs --national-coverage   # Phase 3a US-wide flip: 32 INSERT + 8 UPDATE families in a single
...
function emitMigrationSql(picks, skipFamilies, mode = 'update', colorByFamily = {}, commonNameByFamily = {}, insertSet = null) {
  ...
  if (mode === 'national') {
    ...
    lines.push('--');
    lines.push(`-- Generated by scripts/curation/curate-phylopic.mjs --national-coverage on ${today}`);
    ...
    lines.push('-- family) lives at scripts/curation/phylopic-picks.json under the new + updated');
    lines.push('-- entries.');
    lines.push('--');
    ...
    if (updateFail.length > 0) {
      ...
      // per-row attempts[] cascade lives in scripts/curation/phylopic-picks.json.
      ...
      lines.push('-- attempts[] cascade lives in scripts/curation/phylopic-picks.json. A future');
  ...
  if (mode === 'national-residual') {
    ...
    lines.push('--');
    lines.push(`-- Generated by scripts/curation/curate-phylopic.mjs --rescue-national-residual on ${today}`);
    lines.push(`-- using the family → species → genus cascade against Phylopic build ${PHYLOPIC_BUILD}.`);
    lines.push('-- SPECIES_OVERRIDES carries one iconic representative species per family');
    lines.push('-- (see scripts/curation/curate-phylopic.mjs); each rescued entry carries');
    lines.push('-- `resolutionPath: "family"|"species"|"genus"` + `attempts[]` in');
    lines.push('-- scripts/curation/phylopic-picks.json so a reader can see which lookup path');
  ...
  if (mode === 'rescue') {
    ...
    lines.push('--');
    lines.push(`-- Generated by scripts/curation/curate-phylopic.mjs --rescue-via-species on ${today}`);
    lines.push('-- using the Phylopic API two-step recipe (/nodes?filter_name →');
    ...
    lines.push('-- scripts/curation/curate-phylopic.mjs (with detailed comment on the audit).');
    lines.push('--');
    ...
    lines.push('-- lives at scripts/curation/phylopic-picks.json under the updated entries. Each');
  ...
  if (mode === 'recurate') {
    ...
    lines.push('--');
    lines.push(`-- Generated by scripts/curation/curate-phylopic.mjs --recurate-nulls on ${today}`);
    lines.push('-- using the Phylopic API two-step recipe (/nodes?filter_name →');
    ...
    lines.push('-- per family) lives at scripts/curation/phylopic-picks.json under the updated');
    lines.push('-- entries. The skipFamilies memoization mechanism is now empty and');
    lines.push('-- deprecated — see the comment at the top of scripts/curation/curate-phylopic.mjs.');
    lines.push('--');
    lines.push('-- After this migration lands in main, the operator runs');
    lines.push('-- scripts/purge-silhouettes-cache.sh (#252) as part of the production');
    lines.push('-- deploy runbook to purge the CDN cache for /api/silhouettes.');
    ...
    if (rescued.length === 0) {
      ...
      lines.push('-- family\'s color. Re-run scripts/curation/curate-phylopic.mjs --recurate-nulls');
  ...
  if (mode === 'backfill') {
    ...
    lines.push(`-- Generated by scripts/curation/curate-phylopic.mjs --backfill on ${today}`);
    lines.push('-- using the Phylopic API two-step recipe (/nodes?filter_name →');
    lines.push('-- /images?filter_node) and the auto-pick heuristic (license preference');
    ...
    lines.push('--');
    lines.push('-- The full audit trail (every candidate the heuristic considered, per');
    lines.push('-- family) lives at scripts/curation/phylopic-picks.json under the new entries.');
    lines.push('--');
    lines.push('-- After this migration lands in main, the operator runs');
    lines.push('-- scripts/purge-silhouettes-cache.sh (#252) as part of the production');
    lines.push('-- deploy runbook to purge the CDN cache for /api/silhouettes.');
    ...
    lines.push('-- every seeded AZ bird family. Generated by scripts/curation/curate-phylopic.mjs');
    ...
    lines.push('-- per family) lives at scripts/curation/phylopic-picks.json.');
    lines.push('--');
  ...
  if (mode === 'backfill') {
    ...
    if (failures.length > 0) {
      ...
      lines.push('-- (a) the operator listed them in scripts/curation/phylopic-picks.json#skipFamilies');
      ...
      lines.push('-- network) abort the run instead — see scripts/curation/curate-phylopic.mjs #267.');
...
async function main() {
  ...
  if (failures.length > 0) {
    ...
    console.error('it permanently, add it to scripts/curation/phylopic-picks.json#skipFamilies.');
    console.error('================================================================');
    process.exit(2);
  }
...
// pattern in scripts/curation/silhouette.mjs so a test file can import this module
...
export { emitMigrationSql, escapeSqlString, todayUtc, NATIONAL_INSERT_FAMILIES, NATIONAL_UPDATE_FAMILIES, NATIONAL_COLOR_BY_FAMILY, NATIONAL_COMMON_NAME_BY_FAMILY, NATIONAL_RESIDUAL_FAMILIES, PHYLOPIC_BUILD };
