...
| **P3 — frontend core + ZIP** | C2+C5 · C2a (chooser) · C3+C4+C7 · C6 · D5 | C6 needs C2–C5 + C2a + D5; C7 before D6 | fitBounds spy duration 0 under reduced-motion; one refetch per scope change; **chooser shown when unscoped, map+fetch suppressed**; scoped-empty shows *data* copy not *filter* copy |
...
| **C2a** ⭐NEW | **Landing scope chooser** — the pre-map surface: ZIP input + state `<select>` (co-primary) + a de-emphasized "Explore the whole US map" (→ `?scope=us`). Provides the chooser UI + **emits the chosen scope**; the render-gating (chooser-vs-map + fetch suppression) lives in C6/#740. NOT the on-map bar (that is C4). | `ScopeChooser.tsx`, `styles.css`, test | C2, D5, #732 (`/api/states`), C0 | **CSS sub-task**; a11y; render-gating |
