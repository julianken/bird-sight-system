...
All MapLibre wiring lives inside React components using **`react-map-gl@^7`** (`<Map>`, `<Source>`, `<Layer>`). No imperative `map.addSource` / `map.addLayer` calls. `maplibre-gl@^4` ships as a peer dependency. This is the single rendering path — don't mix imperative MapLibre with the JSX API; the tests at `MapCanvas.test.tsx` assert on `<Source>` prop shape, which is the react-map-gl convention.
