...
- **Source** (`observations`): GeoJSON feature collection built from the already-fetched `Observation[]`. `cluster: true`, `clusterMaxZoom: 14`, `clusterRadius: 50`.
- **Layer 1** (`clusters`): circle, paint color/size by `point_count`.
- **Layer 2** (`cluster-count`): symbol, text from `point_count`.
- **Layer 3** (`unclustered-point`): circle, color by `isNotable`.
...
3. **URL-state swap.** `view` union change + `?view=hotspots` redirect. TDD: unit-test the redirect in `url-state.test.ts` (existing file — there should already be a test file here; verify).
...
- `maplibre-gl` — API shape churns between majors (v4 vs v3 differ on `addSource` signatures and the style spec).
- `react-map-gl` — v7 has a different component API than v6. Pin-read before importing.
