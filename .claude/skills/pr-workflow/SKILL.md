...
2. **Bot review dispatches through the `julianken-bot` Agent subagent.** It loads credentials from macOS Keychain and posts as the `@julianken-bot` collaborator. Never use `gh pr review` from the main session — that posts under Julian's identity and defeats the cross-identity property that branch protection's 1-review rule relies on (a separate GitHub account is what makes the approval count). It also forfeits the cross-tier model-bias reduction (NYU, January 2026) that dispatching to a different Claude tier provides. See `references/julianken-bot-dispatch.md`.
...
- **`knip (informational)` blocks Mergify despite the name.** Branch protection has `knip (informational)` listed as a required status check, even though the GitHub Actions workflow runs `continue-on-error: true`. A knip failure on a PR therefore blocks Mergify's queue progression — the Mergify status comment will read "Waiting for any of: `check-neutral = knip (informational)` / `check-skipped = knip (informational)` / `check-success = knip (informational)`". Resolve before queueing. The fix is almost always to add an ignore rule to `knip.ts` with a dated comment matching the file's existing convention (what it silences, what risk it carries, how a re-audit verifies it). Common false-positive classes: Terraform `file(...)`-loaded files, type-only test files compiled via `tsc -p tsconfig.test.json`, npm-script bin invocations, dynamic require/import. DO NOT delete the file knip is complaining about unless it is genuinely orphan code. The same fix can also be a separate `chore: knip` PR that lands first, after which downstream PRs rebase and inherit the fix.
