...
*For each `?family=F` view, `Σ rendered marker totals ≈ legendSum(view)` (the VIEWPORT-scoped F legend count, NOT `view.network.total`).*
- Filter by family F → the **viewport-scoped** stated count (`legendSum(view)` = the `?family=F` legend's in-view F count) must be conserved by what RENDERS. Filtered to one family there is **no overflow tail** (every cluster is that family — a single-family cluster pills out with its full count), so `Σ marker totals == legend F` when rendering is correct.
- **Stated is the legend, NOT `network.total` (#1283):** the `/api/observations` response is fetched over a bbox that at mid-zoom is **WIDER than the painted viewport**, so `network.total` (the fetch-bbox Σ) exceeds the viewport-painted markers by the served-but-offscreen tail. Comparing that fetch total against viewport markers is the same scope confound the lede has — it false-fires "lost K" for legitimately-served-but-offscreen birds (e.g. Woodpeckers fetch 168 / viewport 148 → a spurious 12% shortfall). The legend IS viewport-scoped, so `legendSum`-vs-`rendered` is the like-with-like comparison. This mirrors **MR-2's `aggregated-response-scopewide`** discipline (legend viewport-scoped, network response-scoped) and **MR-5's lede-vs-viewport** framing. The raw fetch total is surfaced as `numbers.networkTotal` for triage.
- **Violation:** a shortfall beyond `max(2, round(stated·10%))` = the map dropped birds the (viewport) filter counted — the reported "**filter says 7, only 3 render**" bug. Severity high; repro = the filtered `#map=` URL.
...
- **Carve-outs:** `inconclusive` filtered capture (capture failed — not a render verdict; MR-4 still reports the consistency miss separately); `stated == 0` (the legend total is 0 — nothing to render — MR-4(a) covers the 0-vs-baseline case).
...
- **MR-10 (filtered render-completeness — the "filter says N, only M render" catcher, #1274):** for each `?family=F` view, `Σ rendered marker totals ≈ legendSum(view)` (the VIEWPORT-scoped F legend count, NOT `view.network.total` — see #1283). A real `--samples 6` run MISSED the reported "filter says 7, shows 3" bug because MR-2b runs only on the unfiltered ladder and MR-4 is legend-vs-legend — neither compares `stated count vs Σ rendered markers` on a filtered view. MR-10 closes that seam. **Scope correction (#1283):** the first cut compared `view.network.total` (the fetch-bbox Σ, wider than the painted viewport at mid-zoom) against the viewport-painted markers, which false-fired "lost K" for served-but-offscreen birds (Woodpeckers fetch 168 / viewport 148). The fix uses the viewport-scoped `legendSum` as `stated` (like-with-like), mirroring MR-2's `aggregated-response-scopewide` and MR-5's lede-vs-viewport discipline; the raw fetch total is surfaced as `numbers.networkTotal`. Carve-outs: inconclusive capture, `stated == 0` (legend total 0). **Filter-capture hardening lands with it:** probe the **top-4** families (up from 2), distinguish a 200-but-EMPTY filtered response (a REAL "filter returned 0" signal — kept so MR-4/MR-10 flag it) from a fetch timeout (marked `inconclusive`, retried once), and bump `FILTER_BUNDLE_VIEWS` 6→8 (1 unfiltered + 4 family + 3 since) with the pacing-guard estimate.
